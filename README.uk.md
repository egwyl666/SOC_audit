...
Як виглядає запуск (однорядкова команда, аудит хоста без IOC; `...` — кроки, опущені в прикладі для стислості).
...
- Блакитні рядки `[час] N. ...` — кроки збору в порядку NIST (спершу волатильні дані).
- Жовті `УВАГА: ...` — попередження: немає IOC, докази на системному диску, 32-бітний PowerShell, немає прав адміністратора.
- Жовтий рядок `    ! <помилка> [рядок N]` під кроком означає, що крок завершився з помилкою; решта кроків виконується.
  Результати всіх кроків — у `collection_steps.csv` і у звіті (розділ «Цілісність»).
- Наприкінці: шляхи до звіту, папки й архіву, SHA256 маніфесту й архіву (зафіксуйте їх у тікеті) і кількість прапорців.
